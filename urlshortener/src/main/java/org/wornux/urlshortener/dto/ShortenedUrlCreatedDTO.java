...
/**
 * A Data Transfer Object (DTO) representing the creation of a shortened URL.
 * This record encapsulates the original URL, the shortened URL, and a QR code.
 * 
 * @param originalUrl  The original URL to be shortened. Must be a valid URL
 *                     format.
 * @param shortenedUrl The shortened version of the URL. Must not exceed 10
 *                     characters.
 * @param qrCode       The QR code representation of the shortened URL. Cannot
 *                     be blank.
...
public record ShortenedUrlCreatedDTO(String originalUrl, String shortenedUrl, String qrCode) {
   ...
   *                                  - {@code originalUrl} is null, blank, or not
   *                                  a valid URL format.
   *                                  - {@code shortenedUrl} is null, blank, or
   *                                  exceeds 10 characters.
   *                                  - {@code qrCode} is null or blank.
   */
  public ShortenedUrlCreatedDTO {
    ...
    if (!originalUrl.matches("^(https?|ftp)://[^\\s/$.?#].[^\\s]*$")) {
      throw new IllegalArgumentException("Original URL must be a valid URL format (e.g., http://example.com)");
    }
