...
public class FormService {
  ...
  public Optional<Form> updateForm(Long formId, Boolean isSynchronized) {
    ...
    if (!isSynchronized)
      throw new IllegalArgumentException("Form cannot be unsynchronized");

    Optional<Form> form = formRepository.findById(formId).map(f -> {
  ...
  public void deleteForm(Long id) {
    if (id == null)
      throw new IllegalArgumentException("Form ID must not be null");
    else if (getFormById(id).isEmpty())
